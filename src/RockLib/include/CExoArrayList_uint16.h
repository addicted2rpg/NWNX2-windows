...
#define NX_NWN_CEXOARRAYLIST(N, T)                                                                             \
    ...
    static inline int CExoArrayList_ ## N ## _add (CExoArrayList_ ## N *list, T val, int reserve=1) {          \
        if (list->len == list->alloc) {                                                                        \
            T *re;                                                                                             \
            if ((re = (T *)realloc(list->data, sizeof(T) * (list->alloc + reserve))) == NULL)                  \
                return -1;                                                                                     \
            list->data = re;                                                                                   \
            list->alloc +=reserve;                                                                             \
        }                                                                                                      \
    ...
    static inline int CExoArrayList_ ## N ## _delfeatindex (CExoArrayList_ ## N *list, int idx) {              \
        if (idx < 0 || idx >= list->len)                                                                       \
            return -1;                                                                                         \
        if (idx < list->len - 1)                                                                               \
        	list->data[idx] = list->data[list->len-1];                                                         \
        return --(list->len);                                                                                  \
    }                                                                                                          \
...
#define NX_NWN_CEXOARRAYLIST_CONTAINS(N, T)                                                                    \
    ...
    static inline int CExoArrayList_ ## N ## _delfeatvalue (CExoArrayList_ ## N *list, const T val) {          \
        int i;                                                                                                 \
        for (i = 0; i < list->len; i++) {                                                                      \
            if (list->data[i] == val)                                                                          \
                CExoArrayList_ ## N ## _delfeatindex(list, i);                                                 \
        }                                                                                                      \
		if (list->len > -1 || i == list->len) return list->len;                                                \
		else return -1;                                                                                        \
    }                                                                                                          \
...
NX_NWN_CEXOARRAYLIST_CONTAINS(string, CExoLocString)

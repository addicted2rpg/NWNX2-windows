...
// addicted2rpg: This should be 0x005C0380
...
void HookRunScript()
{
	...
	DWORD old_RunScript = FindHookRunScript(); // should be 0x005C0380
	DWORD old_CrossAreaPathFind = FindHookCrossAreaPathFind();

	DWORD old_PathFind = FindHookPathFind();

	if (old_RunScript)
		success = HookFunction(RunScriptHookProc, (PVOID *)&RunScriptNextHook, (PVOID) old_RunScript, 2);
		//success = HookCode((PVOID) old_RunScript, RunScriptHookProc, (PVOID*) &RunScriptNextHook);

	if (old_CrossAreaPathFind)
		success_crossarea = HookFunction(CrossAreaHookProc, (PVOID*) &CrossAreaNextHook, (PVOID) old_CrossAreaPathFind, 3);
		//success_crossarea = HookCode((PVOID) old_CrossAreaPathFind, CrossAreaHookProc, (PVOID*) &CrossAreaNextHook);

	if (old_PathFind)
		success_pathfind = HookFunction(PathfindHookProc, (PVOID*) &PathfindNextHook, (PVOID) old_PathFind, 0);
